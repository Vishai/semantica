...
export class NoteObjectsView extends ItemView {
  ...
  private refresh() {
    ...
    container.empty();
    container.addClass('semantica-objects');
  ...
  private renderSummary(
    ...
    const summary = container.createDiv({ cls: 'semantica-objects-summary' });

    const stats = summary.createDiv({ cls: 'semantica-objects-stats' });
    stats.createEl('span', { text: `${declarations.length} objects` });
    stats.createEl('span', { text: '•', cls: 'semantica-objects-stat-sep' });
    ...
    if (validation.errors.length > 0) {
      ...
      stats.createEl('span', {
        ...
        cls: 'semantica-objects-stat-error',
      });
    }
  }

  private renderValidation(container: Element, validation: any) {
    const section = container.createDiv({ cls: 'semantica-objects-validation' });
    section.createEl('h4', { text: 'Issues' });

    for (const error of validation.errors) {
      const item = section.createDiv({ cls: 'semantica-objects-validation-item' });

      const icon = item.createSpan({ cls: 'semantica-objects-validation-icon' });
      icon.textContent = error.severity === 'error' ? '⚠' : 'ℹ';

      const message = item.createSpan({ cls: 'semantica-objects-validation-message' });
  ...
  private renderObjects(
    ...
    const section = container.createDiv({ cls: 'semantica-objects-list' });
    ...
    for (const decl of declarations) {
      const item = section.createDiv({ cls: 'semantica-objects-item' });

      // DotId badge
      const badge = item.createSpan({ cls: 'semantica-objects-item-badge' });
      badge.textContent = decl.dotId.signature;

      // Term
      const term = item.createSpan({ cls: 'semantica-objects-item-term' });
      term.textContent = decl.term;

      // Reference count
      const count = refCounts.get(decl.dotId.value) || 0;
      const refs = item.createSpan({ cls: 'semantica-objects-item-refs' });
      refs.textContent = count === 0 ? 'unused' : `${count} ref${count > 1 ? 's' : ''}`;
      if (count === 0) refs.addClass('semantica-objects-item-refs-unused');

      // Position (line number)
      const pos = item.createSpan({ cls: 'semantica-objects-item-pos' });
  ...
  private renderGlyphsSummary(container: Element, parseResult: any) {
    ...
    const section = container.createDiv({ cls: 'semantica-objects-glyphs' });
    ...
    const grid = section.createDiv({ cls: 'semantica-objects-glyphs-grid' });
    for (const [symbol, count] of sorted) {
      const item = grid.createDiv({ cls: 'semantica-objects-glyph-item' });

      const symbolSpan = item.createSpan({ cls: 'semantica-objects-glyph-symbol' });
      symbolSpan.textContent = symbol;

      const countSpan = item.createSpan({ cls: 'semantica-objects-glyph-count' });
